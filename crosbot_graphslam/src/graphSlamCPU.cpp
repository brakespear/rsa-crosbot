...
void GraphSlamCPU::updateTrack(Pose icpPose, PointCloudPtr cloud, ros::Time stamp) {
   ...
   double sumTh = fabs(localMaps[currentLocalMap].internalCovar[2][0]) + 
   ...
   } else if (temp > LocalMapDistance * 1.5) {
...
void GraphSlamCPU::addToFreeArea(double px, double py) {
   ...
   double denom = log(1.5);
   for (; n > 0; --n) {
      if (index < 0 || index >= DimLocalOG * DimLocalOG) {
         cout << "Fuck this shit " << index << endl;
      }
...
double GraphSlamCPU::evaluateMapMatch(int ref, int test, double offX, double offY, double offTh) {
   ...
   if (finished) {
      ...
      if (common->combineIndex < 0 && common->matchSuccess > 0) {
         ...
         double score = evaluateMapMatch(currentLocalMap, common->potentialMatches[matchIndex],
               constraintX, constraintY, constraintTh);
         cout << "The matching score is......" << score << endl;
         if (score < FreeAreaThreshold) {
...
void GraphSlamCPU::calculateOptimisationChange(int numIterations) {
   ...
   for (int globalWarp = 0; globalWarp < common->numConstraints; globalWarp++) {
      ...
      cout << globalWarp << " " << localMaps[parentIndex].treeLevel << " Residual is: " << residual[0] << " " << residual[1] << " " << residual[2] << endl;
      mult3x3Matrix(a, b, c);
      a[1][0] *= -1;
      a[0][1] *= -1;
      mult3x3Matrix(c, a, b);
...
void GraphSlamCPU::warpLocalMap(int mapIndex, double errX, double errY, double errTh) {
   ...
   cout << "Num warp points before is: " << localMaps[mapIndex].numWarpPoints << endl;
   ...
   cout << "Num warp points after is: " << localMaps[mapIndex].numWarpPoints << endl;
   delete[] ogMap;
}
